...
* 시작 노드에서 출발해 한 방향으로 갈 수 있는만큼 깊이 탐색한 후,
...
public class A_DFS {
    ...
    public static Integer solution(String input) throws IOException {
        ...
        map = new int[node+1][node+1];  // 0번 인덱스를 제외하고 사용하기 위함 (즉, 편하게 보려고)
...
//        dfsRecursive(1);  // 인자는 시작값 > 1
    ...
    private static void dfsRecursive(int start) {
        ...
        for(int i = 1; i<= node; i++) {
            /* start 정점의 이웃 중 방문하지 않는 이웃을 찾는다. */
            if((map[start][i] == 1) && (!visit[i])) {
                /* 바이러스를 전파할 이;웃 노드 컴퓨터를 찾은 것이므로
