...
/*
* 완전 탐색 (Brute Force) + 백트래킹 (Back Tracking)
...
public class E_BruteForce {
    ...
    public static Integer solution(String input) {
        ...
        int n = st.countTokens();    // StringTokenizer 안에 있는 메소드
        nums = new int[n];
        for (int i = 0; i < n; i++) {
        ...
        backtrack(0, n, new ArrayList<>());    // 0: 깊이를 이야기함
    ...
    private static void backtrack(int depth, int n, ArrayList<Integer> perm) {
        ...
        /* 조건문에 걸리지 않았다는 것은 사용하지 않은 정수를 하나씩
        * 선택하여 재귀적으로 다음 정수를 선택해야 한다는 의미 */
        for (int i = 0; i < n; i++) {
