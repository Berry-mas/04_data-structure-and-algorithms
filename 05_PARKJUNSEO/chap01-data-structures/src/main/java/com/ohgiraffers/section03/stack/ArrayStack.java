...
// 자료구조 중 하나인 "스택(Stack)"을 배열을 이용해서 직접 구현한 코드
/*
* 이 코드가 원하는 것
*
* 후입선출(LIFO) 방식의 자료구조를 직접 구현해서,
* 사용자(혹은 프로그램)가 데이터를 순서대로 쌓고 (push), 꺼내고 (pop), 들여다보고 (peek) 할 수 있도록 하는 유연하고 확장 가능한 스택을 제공하는 것.
*
* 핵심 메시지
* "배열 기반 스택의 핵심 동작을 직접 구현함으로써, 자바의 기본 라이브러리(Stack 클래스 등)를 쓰지 않고도 동작을 완전히 이해하고 조절할 수 있게 해준다."
* */
